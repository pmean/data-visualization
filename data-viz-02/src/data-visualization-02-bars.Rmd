...
y <- data.frame(x= 2, y=-5)
z <- data.frame(x=-5, y= 2)
m <- data.frame(x= 7, y= 7)
d <- 1.2
```
...
df <- data.frame(
  ...
  y=c(ry, gy, by), 
  g=c(rg, gg, bg), 
  a=rep(c(-45, -90, 0), each=n),
  j=rep(c(0.5, 0.5, 0.5), each=n))

ggplot(df, aes(x, y)) + 
  geom_segment(x=k$x, y=k$y, xend=r$x, yend=r$y, col="gray") +
  geom_segment(x=k$x, y=k$y, xend=g$x, yend=g$y, col="gray") +
  geom_segment(x=k$x, y=k$y, xend=b$x, yend=b$y, col="gray") +
  geom_text(col=df$g, label=df$g, angle=df$a, hjust=df$j) +
  geom_text(col="#FFFF00", x=y$x, y=y$y, label="#FFFF00", angle=-67) +
  geom_text(col="#FF00FF", x=z$x, y=z$y, label="#FF00FF", angle=-22) +
  geom_text(col="#00FFFF", x=m$x, y=m$y, label="#00FFFF", angle=-45) +
