...
### Colors, gradients of red to green through gray, through black
...
y1 <- rep(1, 2*n)
y2 <- rep(2, 2*n)
c1 <- c(rgb(q, 0, 0), rgb(0, p, 0))
c2 <- c(rgb(1-p/2, p/2, p/2), rgb(q/2, 1-q/2, q/2))
df <- data.frame(x=rep(x, 2), y=c(y1, y2), co=c(c1, c2))
ggplot(df, aes(x, y)) + 
  geom_point(color=df$co) +
  theme_void() +
  theme(
    panel.background=element_rect(fill="#808080"))
...
hcl_plot <- function(lum) {
  ...
  g <- ggplot(dg3, aes(x, y)) + 
    ...
    geom_text(x=d*cos( 0*pi/3), y=d*sin( 0*pi/3), angle= 90, label=  "0 = Red",     color="red") + 
    geom_text(x=d*cos(-1*pi/3), y=d*sin(-1*pi/3), angle= 30, label= "60 = Yellow",  color="yellow") + 
    geom_text(x=d*cos(-2*pi/3), y=d*sin(-2*pi/3), angle=-30, label="120 = Green",   color="green") + 
    geom_text(x=d*cos(-3*pi/3), y=d*sin(-3*pi/3), angle=-90, label="180 = Cyan",    color="cyan") + 
    geom_text(x=d*cos(-4*pi/3), y=d*sin(-4*pi/3), angle= 30, label="240 = Blue",    color="blue") + 
    geom_text(x=d*cos(-5*pi/3), y=d*sin(-5*pi/3), angle=-30, label="300 = Magenta", color="magenta") + 
    theme_void() +
    theme(
      panel.background=element_rect(fill="#808080")
    )
