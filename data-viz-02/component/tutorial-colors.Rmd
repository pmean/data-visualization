...
### Colors, R color schemes (2/3)

![Display of various sequential color schemes](../images/r/color-brewer-sequential.png)

<div class="notes>

These are what color brewer refers to as sequential color schemes. They represent a single color, going from light to dark.
...
```{r display_functions}
display_linear_gradient <- function(co, n=length(co)) {
...
display_circular_gradient <- function(co, n=length(co)) {
  t <- seq(0, 2*pi, length=n)
  x <- cos(t)
  y <- sin(t)
  x0 <- 0.5*x
...
initiate_image()
n <- 300
display_linear_gradient(hcl.colors(300))
finalize_image()
```
...
```{r rainbow-gradient}
initiate_image()
display_circular_gradient(rainbow(300))
finalize_image()
```
...
ggplot(dg4, aes(x, y)) + 
  ...
  expand_limits(y=c(-5, 1)) + 
  expand_limits(x=c(-1.2, 1.2)) + 
  geom_text(x=x4[100], y=y4[100], label="hsv(1/6, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[200], y=y4[200], label="hsv(1/3, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[300], y=y4[300], label="hsv(1/2, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[400], y=y4[400], label="hsv(2/3, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[500], y=y4[500], label="hsv(5/6, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[600], y=y4[600], label="hsv(0 or 1, 1, 1)", color="white", size=sz) + 
  theme_void() +
  theme(
    panel.background=element_rect(fill="#808080")
  )
...
ggplot(dg6, aes(x, y)) + 
  ...
  geom_text(x=x4[500], y=y4[500], label="hsv(5/6, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[600], y=y4[600], label="hsv(0 or 1, 1, 1)", color="white", size=sz) + 
  geom_text(x=x4[100], y=y4[100]-4, label="hsv(1/6, 1, 0)", color="white", size=sz) + 
  geom_text(x=x4[200], y=y4[200]-4, label="hsv(1/3, 1, 0)", color="white", size=sz) + 
  geom_text(x=x4[300], y=y4[300]-4, label="hsv(1/2, 1, 0)", color="white", size=sz) + 
  geom_text(x=x4[400], y=y4[400]-4, label="hsv(2/3, 1, 0)", color="white", size=sz) + 
  geom_text(x=x4[500], y=y4[500]-4, label="hsv(5/6, 1, 0)", color="white", size=sz) + 
  geom_text(x=x4[600], y=y4[600]-4, label="hsv(0 or 1, 1, 0)", color="white", size=sz) + 
  theme_void() +
  theme(
    panel.background=element_rect(fill="#808080")
  )
...
+ Very similar systems are CIE-LUV, CIE-LAB
  
<div class="notes">

Another color system is the HCL system. It is very similar to standards CIE-LUV and CIE-LAB.

It has a hue value, just like the hsl system.

Chroma is a measure of how intense the color is. It is like saturation, but represents a relative amount of saturation, relative to a grey value of the same luminence.

Luminence is a meausre of brightness or lightness. The luminence is similar to the V (value)
...
For a given value of luminence, there may be restrictions in how far you can move towards the pure colors. For high values of luminence, you can't get near the pure red or pure blue colors, as they don't have enough brightness, especially compared to the yellows and greens. For values of low luminence, the opposite occurs. Pure yellow and pure green are just too bright, but you can get reasonable values for something close to pure red and pure blue.
...
These are the color values with a luminence of 90%. It favors the green and yellow side of the color wheel.
