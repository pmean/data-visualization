...
### Colors, Pure color ciircle and alternatives (3/3)
...
### Recommendations, same luminance, equally spaced hue

```{r hcl-circle}
hcl_circle <- function(lum, rad) {
  theta <- seq(0, 1, length=360)
  c2 <- hcl(360*theta, 100*rad, 100*lum, fixup=FALSE)
  dg2 <- data.frame(x=-cos(2*pi*theta), y=-sin(2*pi*theta), co=c2)
  m <- sum(is.na(dg2$co))
  if (m > 0) cat(paste0(m, " missing colors."))
  dg3 <- rbind(dg2[!is.na(dg2$co), ])
  g <- ggplot(dg3, aes(x, y)) + 
    geom_point(col=dg3$co) +
    geom_point(x=dg2$x[360], y=dg2$y[360], color=c2[360], size=20) +
    geom_point(x=dg2$x[ 72], y=dg2$y[ 72], color=c2[ 72], size=20) +
    geom_point(x=dg2$x[144], y=dg2$y[144], color=c2[144], size=20) +
    geom_point(x=dg2$x[216], y=dg2$y[216], color=c2[216], size=20) +
    geom_point(x=dg2$x[288], y=dg2$y[288], color=c2[288], size=20) +
...
### Recommendations, equally spaced hue, luminance

```{r darker-hcl-circle}
initiate_image()
hcl_circle(11/20, rad=0.40)
finalize_image()
```
