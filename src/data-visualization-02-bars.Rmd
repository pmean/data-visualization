...
![Kaggle datasets webpage](../images/02/kaggle-scotus-data.png)

```{r clean, eval=FALSE}
scotus <- read_csv("../data/opinions_since_1970.csv")
scotus %<>% filter(grepl("^justice ", author_name)) %>% select(-14)
write.csv(scotus, file="../data/scotus_opinions.csv", row.names=FALSE)
```
...
scotus <- read_csv(file="../data/scotus_opinions.csv")
ggplot(scotus, aes(year_filed)) +
  geom_bar()
```
